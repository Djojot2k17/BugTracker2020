...
namespace BugTracker2020.Services
{
  public class BTAccessService : IBTAccessService
  {
    ...
    public async Task<bool> CanAccessProject(string userId, int projectId, string roleName)
    {
      switch(roleName)
      {
        case "Admin":
    ...
    public async Task<bool> CanAccessTicket(string userId, int ticketId, string roleName)
    {
      bool result = false;
      switch (roleName)
      {
